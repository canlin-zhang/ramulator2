...
class BHO3Core : public Clocked<BHO3Core>
{
    ...
    class InstWindow
    {
      ...
      private:
        ...
        std::vector<bool>
            m_ready_list; // Bitvector to mark whether each instruction is ready to be retired.
        std::vector<Addr_t> m_addr_list; // Which address is each LD/ST instruction targeting? TODO:
                                         // Perf. optimization with unordered map?
      ...
      public:
        ...
        /**
         * @brief   Set a memory instruction to ready. Called by the callback when a request is
         * served by the memory
         *
  ...
  public:
    BHO3Core(int id, int ipc, int depth, size_t num_expected_insts, uint64_t num_max_cycles,
             std::string trace_path, ITranslation* translation, BHO3LLC* llc, int lat_hist_sens,
             std::string& dump_path, bool is_attacker);
