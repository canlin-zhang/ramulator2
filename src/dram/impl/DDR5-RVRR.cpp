...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, DDR5RVRR, "DDR5-RVRR",
                                      "DDR5 with Reduced Victim Row Refresh")
  private:
    int m_RH_radius = -1;
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
        //   name         rate   nBL  nCL nRCD   nRP  nRAS   nRC   nWR  nRTP nCWL nPPD nCCDS
        //   nCCDS_WR nCCDS_WTR nCCDL nCCDL_WR nCCDL_WTR nRRDS nRRDL nFAW nRFC1 nRFC2 nRFCsb nREFI
        //   nREFSBRD nRFM1 nRFM2 nRFMsb nRRFMsb nDRFMab nDRFMsb nVRR nRVRR nCS, tCK_ps
        {"DDR5_3200AN",
         {3200, 8,  24, 24, 24, 52, 75, 48, 12, 22, 2,  8,  8,  22 + 8 + 4, 8,  16, 22 + 8 + 16, 8,
          -1,   -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1, -1,         -1, 2,  625}},
        {"DDR5_3200BN",
         {3200, 8,  26, 26, 26, 52, 77, 48, 12, 24, 2,  8,  8,  24 + 8 + 4, 8,  16, 24 + 8 + 16, 8,
          -1,   -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1, -1,         -1, 2,  625}},
        {"DDR5_3200C",
         {3200, 8,  28, 28, 28, 52, 79, 48, 12, 26, 2,  8,  8,  26 + 8 + 4, 8,  16, 26 + 8 + 16, 8,
          -1,   -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1, -1,         -1, 2,  625}},
    };
    ...
    inline static const std::map<std::string, std::vector<double>> current_presets = {
        // name           IDD0  IDD2N   IDD3N   IDD4R   IDD4W   IDD5B   IPP0  IPP2N  IPP3N  IPP4R
        // IPP4W  IPP5B
    ...
    inline static constexpr ImplDef m_commands = {
        "ACT",        "PRE",       "PREA",    "PREsb",     "RD",         "WR",         "RDA",
        "WRA",        "REFab",     "REFsb",   "REFab_end", "REFsb_end",  "RFMab",      "RFMsb",
        "RFMab_end",  "RFMsb_end", "DRFMab",  "DRFMsb",    "DRFMab_end", "DRFMsb_end", "RRFMsb",
        "RRFMsb_end", "VRR",       "VRR_end", "RVRR",      "RVRR_end",
    };
        ...
        LUT(m_commands, m_levels,
            {
                ...
                {"PREsb", "bank"},      {"RD", "column"},       {"WR", "column"},
                {"RDA", "column"},      {"WRA", "column"},      {"REFab", "rank"},
                {"REFsb", "bank"},      {"REFab_end", "rank"},  {"REFsb_end", "bank"},
                {"RFMab", "rank"},      {"RFMsb", "bank"},      {"RFMab_end", "rank"},
                {"RFMsb_end", "bank"},  {"DRFMab", "rank"},     {"DRFMsb", "bank"},
                {"DRFMab_end", "rank"}, {"DRFMsb_end", "bank"}, {"RRFMsb", "bank"},
                {"RRFMsb_end", "bank"}, {"VRR", "bank"},        {"VRR_end", "bank"},
    ...
    inline static constexpr ImplDef m_timings = {
        ...
        "nWR",     "nRTP",     "nCWL",      "nPPD",     "nCCDS", "nCCDS_WR", "nCCDS_WTR",
        "nCCDL",   "nCCDL_WR", "nCCDL_WTR", "nRRDS",    "nRRDL", "nFAW",     "nRFC1",
        "nRFC2",   "nRFCsb",   "nREFI",     "nREFSBRD", "nRFM1", "nRFM2",    "nRFMsb",
        "nRRFMsb", "nDRFMab",  "nDRFMsb",   "nVRR",     "nRVRR", "nCS",      "tCK_ps"};

    /************************************************
     *                   Power
     ***********************************************/
    inline static constexpr ImplDef m_voltages = {"VDD", "VPP"};

    inline static constexpr ImplDef m_currents = {"IDD0",  "IDD2N", "IDD3N", "IDD4R",
                                                  "IDD4W", "IDD5B", "IPP0",  "IPP2N",
                                                  "IPP3N", "IPP4R", "IPP4W", "IPP5B"};

    inline static constexpr ImplDef m_cmds_counted = {"ACT", "PRE", "RD",   "WR",  "REF",
                                                      "RFM", "VRR", "RRFM", "RVRR"};

    /************************************************
     *                 Node States
     ***********************************************/
    inline static constexpr ImplDef m_states = {"Opened", "Closed", "PowerUp", "N/A", "Refreshing"};
  ...
  public:
    ...
    {
        Node(DDR5RVRR* dram, Node* parent, int level, int id)
            : DRAMNodeBase<DDR5RVRR>(dram, parent, level, id){};
    };
  ...
  public:
    ...
    void init() override
    {
        ...
        m_latency_factor_vrr = param<float>("latency_factor_vrr")
                                   .desc("Factor to scale the latency of the DRAM.")
                                   .default_val(1.0f);
        m_latency_factor_rfc = param<float>("latency_factor_rfc")
                                   .desc("Factor to scale the latency of the DRAM.")
                                   .default_val(1.0f);
        set_organization();
        set_timing_vals();
  ...
  private:
    ...
    void set_timing_vals()
    {
        ...
        m_RH_radius = param<int>("RH_radius")
                          .desc("The number of rows to refresh on each side")
                          .default_val(2);
        ...
        m_timing_vals("nRVRR") =
            m_latency_factor_vrr * ((float)m_timing_vals("nVRR") * (float)m_timing_vals("nRAS") /
                                    (float)(m_timing_vals("nRAS") + m_timing_vals("nRP"))) +
            ((float)m_timing_vals("nVRR") * (float)m_timing_vals("nRP") /
             (float)(m_timing_vals("nRAS") + m_timing_vals("nRP")));
        m_timing_vals("nRRFMsb") =
            m_latency_factor_vrr * ((float)m_timing_vals("nRFMsb") * (float)m_timing_vals("nRAS") /
                                    (float)(m_timing_vals("nRAS") + m_timing_vals("nRP"))) +
            ((float)m_timing_vals("nRFMsb") * (float)m_timing_vals("nRP") /
             (float)(m_timing_vals("nRAS") + m_timing_vals("nRP")));
        ...
        m_timing_vals("nDRFMab") =
            2 * m_BRC * JEDEC_rounding_DDR5(tRRFsb_TABLE[0][density_id], tCK_ps);
        m_timing_vals("nDRFMsb") =
            2 * m_BRC * JEDEC_rounding_DDR5(tRRFsb_TABLE[1][density_id], tCK_ps);
        ...
        for (int i = 1; i < m_timings.size() - 1; i++)
        {
            ...
            else if (auto provided_timing = param_group("timing")
        ...
        if (m_latency_factor_rfc != 1.0f)
            m_timing_vals("nRFC1") =
                m_latency_factor_rfc *
                    ((float)m_timing_vals("nRFC1") * (float)m_timing_vals("nRAS") /
                     (float)(m_timing_vals("nRAS") + m_timing_vals("nRP"))) +
                ((float)m_timing_vals("nRFC1") * (float)m_timing_vals("nRP") /
                 (float)(m_timing_vals("nRAS") + m_timing_vals("nRP")));
...
#define V(timing) (m_timing_vals(timing))
        ...
        populate_timingcons(
            this,
            {
                ...
                // Two-Cycle Commands
                {.level = "channel",
                 .preceding = {"ACT", "RD", "RDA", "WR", "WRA"},
                 .following = all_commands,
                 .latency = 2},
                ...
                {.level = "rank",
                 ...
                 .following = {"WR", "WRA"},
                 .latency = V("nCCDS_WR")},
                /// RD <-> WR, Minimum Read to Write, Assuming Read DQS Offset = 0, tRPST = 0.5,
                /// tWPRE = 2 tCK
                {.level = "rank",
                 .preceding = {"RD", "RDA"},
                 .following = {"WR", "WRA"},
                 .latency = V("nCL") + V("nBL") + 2 - V("nCWL") + 2}, // nCCDS_RTW
                ...
                {.level = "rank",
                 .preceding = {"PREA"},
                 .following = {"ACT", "VRR", "RVRR"},
                 .latency = V("nRP")},
                /// RAS <-> REF
                {.level = "rank",
                 .preceding = {"ACT"},
                 .following = {"REFab", "RFMab", "DRFMab"},
                 .latency = V("nRC")},
                {.level = "rank",
                 .preceding = {"VRR"},
                 .following = {"REFab", "RFMab", "DRFMab"},
                ...
                {.level = "rank",
                 .preceding = {"PREA"},
                 .following = {"REFab", "RFMab", "DRFMab", "REFsb", "RFMsb", "DRFMsb", "RRFMsb"},
                 .latency = V("nRP")},
                {.level = "rank",
                 .preceding = {"RDA"},
                 .following = {"REFab", "RFMab", "DRFMab"},
                ...
                {.level = "rank",
                 .preceding = {"REFab"},
                 .following = {"ACT", "VRR", "RVRR", "PREA", "REFab", "RFMab", "DRFMab", "REFsb",
                               "RFMsb", "DRFMsb", "RRFMsb"},
                 .latency = V("nRFC1")},
                {.level = "rank",
                 .preceding = {"RFMab"},
                ...
                {.level = "rank",
                 .preceding = {"REFsb"},
                 .following = {"PREA", "REFab", "RFMab", "DRFMab"},
                 .latency = V("nRFCsb")},
                {.level = "rank",
                 .preceding = {"RFMsb"},
                 .following = {"PREA", "REFab", "RFMab", "DRFMab"},
                ...
                {.level = "bank",
                 .preceding = {"ACT"},
                 .following = {"ACT", "VRR", "RVRR", "REFsb", "RFMsb", "DRFMsb", "RRFMsb"},
                 .latency = V("nRC")},
                {.level = "bank",
                 .preceding = {"VRR"},
                 .following = {"ACT", "VRR", "RVRR", "REFsb", "RFMsb", "DRFMsb", "RRFMsb"},
                ...
                {.level = "bank",
                 .preceding = {"WR"},
                 .following = {"RDA"},
                 .latency = V("nCWL") + V("nBL") + V("nWR") - V("nRTP")},

                /// Same-bank refresh/RFM timings. The timings of the bank in other BGs will be
                /// updated by action function
                {.level = "bank",
                 .preceding = {"REFsb"},
                 .following = {"ACT", "VRR", "RVRR", "REFsb", "RFMsb", "DRFMsb", "RRFMsb"},
                 .latency = V("nRFCsb")},
...
#undef V
    ...
    void set_actions()
    {
        ...
        m_actions[m_levels["rank"]][m_commands["REFab_end"]] =
            Lambdas::Action::Rank::REFab_end<DDR5RVRR>;
        m_actions[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Action::Rank::REFab<DDR5RVRR>;
        m_actions[m_levels["rank"]][m_commands["RFMab_end"]] =
            Lambdas::Action::Rank::REFab_end<DDR5RVRR>;
        m_actions[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Action::Rank::REFab<DDR5RVRR>;
        m_actions[m_levels["rank"]][m_commands["DRFMab_end"]] =
            Lambdas::Action::Rank::REFab_end<DDR5RVRR>;

        // Same-Bank Actions.
        m_actions[m_levels["bankgroup"]][m_commands["PREsb"]] =
            Lambdas::Action::BankGroup::PREsb<DDR5RVRR>;

        // We call update_timing for the banks in other BGs here
        m_actions[m_levels["bankgroup"]][m_commands["REFsb"]] =
            Lambdas::Action::BankGroup::REFsb<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["REFsb_end"]] =
            Lambdas::Action::BankGroup::REFsb_end<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["RFMsb"]] =
            Lambdas::Action::BankGroup::REFsb<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["RFMsb_end"]] =
            Lambdas::Action::BankGroup::REFsb_end<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["DRFMsb"]] =
            Lambdas::Action::BankGroup::REFsb<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["DRFMsb_end"]] =
            Lambdas::Action::BankGroup::REFsb_end<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["RRFMsb"]] =
            Lambdas::Action::BankGroup::REFsb<DDR5RVRR>;
        m_actions[m_levels["bankgroup"]][m_commands["RRFMsb_end"]] =
            Lambdas::Action::BankGroup::REFsb_end<DDR5RVRR>;
        ...
        m_actions[m_levels["bank"]][m_commands["VRR_end"]] =
            Lambdas::Action::Bank::VRR_end<DDR5RVRR>;
        m_actions[m_levels["bank"]][m_commands["RVRR"]] = Lambdas::Action::Bank::VRR<DDR5RVRR>;
        m_actions[m_levels["bank"]][m_commands["RVRR_end"]] =
            Lambdas::Action::Bank::VRR_end<DDR5RVRR>;
    };
    ...
    void set_preqs()
    {
        ...
        m_preqs[m_levels["rank"]][m_commands["REFab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5RVRR>;
        m_preqs[m_levels["rank"]][m_commands["RFMab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5RVRR>;
        m_preqs[m_levels["rank"]][m_commands["DRFMab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5RVRR>;

        // Same-Bank Preqs.
        m_preqs[m_levels["rank"]][m_commands["REFsb"]] =
            Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5RVRR>;
        m_preqs[m_levels["rank"]][m_commands["RFMsb"]] =
            Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5RVRR>;
        m_preqs[m_levels["rank"]][m_commands["DRFMsb"]] =
            Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5RVRR>;
        m_preqs[m_levels["rank"]][m_commands["RRFMsb"]] =
            Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5RVRR>;
        ...
        m_preqs[m_levels["bank"]][m_commands["VRR"]] =
            Lambdas::Preq::Bank::RequireBankClosed<DDR5RVRR>;
        m_preqs[m_levels["bank"]][m_commands["RVRR"]] =
            Lambdas::Preq::Bank::RequireBankClosed<DDR5RVRR>;
        m_preqs[m_levels["bank"]][m_commands["ACT"]] =
            Lambdas::Preq::Bank::RequireRowOpen<DDR5RVRR>;
        m_preqs[m_levels["bank"]][m_commands["PRE"]] =
            Lambdas::Preq::Bank::RequireBankClosed<DDR5RVRR>;
    };
    ...
    void set_powers()
    {
        ...
        // m_powers[m_levels["rank"]][m_commands["REFsb_end"]] =
        // Lambdas::Power::Rank::REFsb_end<DDR5RVRR>;
        m_powers[m_levels["rank"]][m_commands["RFMsb"]] = Lambdas::Power::Rank::RFMsb<DDR5RVRR>;
        m_powers[m_levels["rank"]][m_commands["RFMsb_end"]] =
            Lambdas::Power::Rank::RFMsb_end<DDR5RVRR>;
        m_powers[m_levels["rank"]][m_commands["RRFMsb"]] = Lambdas::Power::Rank::RRFMsb<DDR5RVRR>;
        m_powers[m_levels["rank"]][m_commands["RRFMsb_end"]] =
            Lambdas::Power::Rank::RRFMsb_end<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["DRFMsb"]] = Lambdas::Power::Rank::REFsb<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["DRFMsb_end"]] =
        // Lambdas::Power::Rank::REFsb_end<DDR5RVRR>;
        ...
        m_powers[m_levels["rank"]][m_commands["REFab_end"]] =
            Lambdas::Power::Rank::REFab_end<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["RFMab"]] = Lambdas::Power::Rank::REFab<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["RFMab_end"]] =
        // Lambdas::Power::Rank::REFab_end<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["DRFMab"]] = Lambdas::Power::Rank::REFab<DDR5RVRR>;
        // m_powers[m_levels["rank"]][m_commands["DRFMab_end"]] =
        // Lambdas::Power::Rank::REFab_end<DDR5RVRR>;
        ...
        m_powers[m_levels["rank"]][m_commands["RVRR_end"]] =
            Lambdas::Power::Rank::VRR_end<DDR5RVRR>;
    ...
    void finalize() override
    {
        ...
        for (int i = 0; i < num_channels; i++)
        {
            for (int j = 0; j < num_ranks; j++)
            {
                process_rank_energy(m_power_stats[i * num_ranks + j],
                                    m_channels[i]->m_child_nodes[j]);
            }
        }
    }
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node)
    {
        ...
        double act_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) *
            rank_stats.cmd_counters[m_cmds_counted("ACT")] * TS("nRAS") * tCK_ns / 1E3;

        double pre_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD2N")) + VE("VPP") * (CE("IPP0") - CE("IPP2N"))) *
            rank_stats.cmd_counters[m_cmds_counted("PRE")] * TS("nRP") * tCK_ns / 1E3;

        double rd_cmd_energy =
            (VE("VDD") * (CE("IDD4R") - CE("IDD3N")) + VE("VPP") * (CE("IPP4R") - CE("IPP3N"))) *
            rank_stats.cmd_counters[m_cmds_counted("RD")] * TS("nBL") * tCK_ns / 1E3;

        double wr_cmd_energy =
            (VE("VDD") * (CE("IDD4W") - CE("IDD3N")) + VE("VPP") * (CE("IPP4W") - CE("IPP3N"))) *
            rank_stats.cmd_counters[m_cmds_counted("WR")] * TS("nBL") * tCK_ns / 1E3;

        double ref_cmd_energy = (VE("VDD") * (CE("IDD5B")) + VE("VPP") * (CE("IPP5B"))) *
                                rank_stats.cmd_counters[m_cmds_counted("REF")] * TS("nRFC1") *
                                tCK_ns / 1E3;

        double rfm_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) *
            num_bankgroups * rank_stats.cmd_counters[m_cmds_counted("RFM")] * TS("nRFMsb") *
            tCK_ns / 1E3;

        double rrfm_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) *
            num_bankgroups * rank_stats.cmd_counters[m_cmds_counted("RRFM")] * TS("nRRFMsb") *
            tCK_ns / 1E3;

        double vrr_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) *
            rank_stats.cmd_counters[m_cmds_counted("VRR")] * TS("nVRR") * tCK_ns / 1E3;

        double rvrr_cmd_energy =
            (VE("VDD") * (CE("IDD0") - CE("IDD3N")) + VE("VPP") * (CE("IPP0") - CE("IPP3N"))) *
            rank_stats.cmd_counters[m_cmds_counted("RVRR")] * TS("nRVRR") * tCK_ns / 1E3;

        rank_stats.total_background_energy =
            rank_stats.act_background_energy + rank_stats.pre_background_energy;
        rank_stats.total_cmd_energy = act_cmd_energy + pre_cmd_energy + rd_cmd_energy +
                                      wr_cmd_energy + ref_cmd_energy + rfm_cmd_energy +
                                      rrfm_cmd_energy + vrr_cmd_energy + rvrr_cmd_energy;
        ...
        s_total_rfm_cycles[rank_stats.rank_id] =
            rank_stats.cmd_counters[m_cmds_counted("RFM")] * TS("nRFMsb");
        s_total_rrfm_cycles[rank_stats.rank_id] =
            rank_stats.cmd_counters[m_cmds_counted("RRFM")] * TS("nRRFMsb");
        s_total_vrr_cycles[rank_stats.rank_id] =
            rank_stats.cmd_counters[m_cmds_counted("VRR")] * TS("nVRR");
        s_total_rvrr_cycles[rank_stats.rank_id] =
            rank_stats.cmd_counters[m_cmds_counted("RVRR")] * TS("nRVRR");
    }
};
