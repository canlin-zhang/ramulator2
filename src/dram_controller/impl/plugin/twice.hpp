...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IControllerPlugin, TWiCeIdeal, "TWiCe-Ideal",
                                      "Idealized TWiCe.")
  ...
  public:
    void init() override
    {
        ...
        m_twice_pruning_interval_threshold =
            param<float>("twice_pruning_interval_threshold").required();
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        {
            throw ConfigurationError("TWiCe is not compatible with the DRAM implementation that "
        ...
        if (request_found)
        {
                ...
                m_dram->m_command_scopes(req_it->command) == m_rank_level)
            {
                ...
                for (int i = 0; i < m_num_ranks * m_num_banks_per_rank; i++)
                {
                    ...
                    {
                        if (it->second.act_count <
                            it->second.life * m_twice_pruning_interval_threshold)
                        {
                            // Store the entries to be pruned
                            to_be_pruned.emplace_back(it);
                            if (m_is_debug)
                            {
                                std::cout << "TWiCeIdeal: Pruned entry " << it->first
                                          << " from bank " << i << std::endl;
                            }
                        }
                        else
                        {
                            ...
                            if (m_is_debug)
                            {
                                std::cout << "TWiCeIdeal: Incremented life of entry " << it->first
                                          << " in bank " << i << std::endl;
                            }
                        }
                    }
                     ...
                     m_dram->m_command_scopes(req_it->command) == m_row_level)
            {
                ...
                if (m_twice_table[flat_bank_id].find(row_id) == m_twice_table[flat_bank_id].end())
                {
                    ...
                    if (m_is_debug)
                    {
                        std::cout << "TWiCeIdeal: Inserted row " << row_id << " into bank "
                ...
                else
                {
                    ...
                    if (m_twice_table[flat_bank_id][row_id].act_count >= m_twice_rh_threshold)
                    {
                        ...
                        if (m_is_debug)
                        {
                            ...
                            std::cout << "  └  " << "rank: " << req_it->addr_vec[m_rank_level]
                                      << std::endl;
                            std::cout << "  └  "
                                      << "bank_group: " << req_it->addr_vec[m_rank_level + 1]
                                      << std::endl;
                            std::cout << "  └  " << "bank: " << req_it->addr_vec[m_bank_level]
                                      << std::endl;
                            std::cout << "  └  " << "index: " << flat_bank_id << std::endl;
                            std::cout << "TWiCeIdeal: Erased entry " << row_id << " from bank "
                                      << flat_bank_id << std::endl;
                        }
                    }
                }
