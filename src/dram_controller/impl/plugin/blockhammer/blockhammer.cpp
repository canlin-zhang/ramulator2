...
BlockHammer::BaseFilter* BlockHammer::get_bank_filter(Ramulator::Request& req)
{
...
void BlockHammer::setup(IFrontEnd* frontend, IMemorySystem* memory_system)
{
    ...
    if (m_bf_hist_size < 0)
    {
        std::cout << "[Ramulator::BlockHammerPlugin] Row History Buffer size must be positive."
                  << std::endl;
        exit(0);
    }
    ...
    for (int i = 0; i < m_num_ranks * m_num_banks_per_rank; i++)
    {
        ...
        for (int j = 0; j < m_bf_num_filters; j++)
        {
            sub_filters->push_back(
                new SubFilter(m_bf_ctr_count, m_bf_ctr_thresh, m_bf_ctr_saturate, *hash_functions));
        }
    ...
    m_attack_throttler = new AttackThrottler(m_llc, m_bf_num_rh, m_bf_ctr_thresh,
                                             m_bf_len_epoch_clk, m_bf_trefw, m_bf_num_filters);
...
void BlockHammer::update(bool request_found, ReqBuffer::iterator& req_it)
{
    m_clk++;
...
bool BlockHammer::is_act_safe(Request& req)
{
