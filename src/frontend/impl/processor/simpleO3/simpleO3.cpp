...
class SimpleO3 final : public IFrontEnd, public Implementation
{
  ...
  public:
    void init() override
    {
        ...
        // Create the LLC
        m_llc =
            new SimpleO3LLC(llc_latency, llc_capacity_per_core * m_num_cores, llc_linesize_bytes,
                            llc_associativity, llc_num_mshr_per_core * m_num_cores);
        ...
        {
            SimpleO3Core* core = new SimpleO3Core(id, ipc, depth, m_num_expected_insts,
                                                  trace_list[id], m_translation, m_llc);
        ...
        {
            // register_stat(m_cores[core_id]->s_insts_retired).name("cycles_retired_core_{}",
            // core_id);
            register_stat(m_cores[core_id]->s_cycles_recorded)
                .name("cycles_recorded_core_{}", core_id);
