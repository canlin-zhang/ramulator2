...
template <class T> void populate_timingcons(T* spec, std::vector<TimingConsInitializer> initializer)
{
    spec->m_timing_cons.resize(T::m_levels.size(),
...
template <int N, typename V> ImplLUT(ImplDef<N>, std::map<std::string_view, V>) -> ImplLUT<N, 0, V>;
...
template <int N, int M>
ImplLUT<N, M, int> LUT(const ImplDef<N>& key_def, const ImplDef<M>& value_def,
                       const std::map<std::string_view, std::string_view>& lut)
{
