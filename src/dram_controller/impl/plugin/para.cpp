...
class PARA : public IControllerPlugin, public Implementation
{
  ...
  public:
    void init() override
    {
        m_pr_threshold = param<float>("threshold")
                             .desc("Probability threshold for issuing neighbor row refresh")
                             .required();
        if (m_pr_threshold <= 0.0f || m_pr_threshold >= 1.0f)
            throw ConfigurationError("Invalid probability threshold ({}) for PARA!",
                                     m_pr_threshold);
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        {
            throw ConfigurationError("PARA is not compatible with the DRAM implementation that "
