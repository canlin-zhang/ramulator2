...
class RRS : public IControllerPlugin, public Implementation
{
  ...
  private:
    ...
    // per bank row indirection table is implemented in
    // 'src/addr_mapper/impl/linear_mappers_with_rit.cpp'
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        if (request_found)
        {
                ...
                m_dram->m_command_scopes(req_it->command) == m_row_level)
            {
                ...
                // Check HRT
                if (m_hot_row_tracker[flat_bank_id].find(row_id) ==
                    m_hot_row_tracker[flat_bank_id].end())
                {
                    if (m_is_debug)
                    {
                    ...
                    if (m_hot_row_tracker[flat_bank_id].size() < m_num_hrt_entries)
                    {
                        if (m_is_debug)
                        {
                            std::cout << "  └  " << "HRT is not full, inserting with count 1."
                                      << std::endl;
                        }
                    ...
                    else
                    {
                        if (m_is_debug)
                        {
                            std::cout << "  └  " << "HRT is full, searching for a row to evict."
                                      << std::endl;
                        }
                        ...
                        for (auto it = m_hot_row_tracker[flat_bank_id].begin();
                             it != m_hot_row_tracker[flat_bank_id].end(); it++)
                        {
                            ...
                            if (it->second == m_spillover_counter[flat_bank_id])
                            {
                                if (m_is_debug)
                                {
                                    std::cout << "  └  " << "found a row to evict: " << it->first
                                              << std::endl;
                                }
                        ...
                        if (found)
                        {
                            if (m_is_debug)
                            {
                                std::cout << "Removing row " << to_remove << " from HRT."
                                          << std::endl;
                        ...
                        else
                        {
                            ...
                            {
                                std::cout << "  └  "
                                          << "no row to evict, incrementing spillover counter."
                                          << std::endl;
                            }
                ...
                else
                {
                    ...
                    {
                        std::cout << "  └  " << "row " << row_id
                                  << " in HRT. Incrementing its counter." << std::endl;
                    }
                ...
                {
                    std::cout << "==========================" << std::endl;
                    std::cout << "HRT[" << flat_bank_id
                              << "].size(): " << m_hot_row_tracker[flat_bank_id].size()
                              << std::endl;
                    for (auto entry : m_hot_row_tracker[flat_bank_id])
                    {
                        std::cout << entry.first << ":\t" << entry.second << std::endl;
                    }
                    std::cout << "Spillover counter: " << m_spillover_counter[flat_bank_id]
                              << std::endl;
                    std::cout << "==========================" << std::endl;
                }
                ...
                if (m_is_debug)
                {
                    ...
                    std::cout << "  └  " << "count: " << m_hot_row_tracker[flat_bank_id][row_id]
                              << std::endl;
                }
                if (m_hot_row_tracker[flat_bank_id][row_id] % m_rss_threshold == 0)
                {
                    ...
                    if (prev_swapped_row != -1)
                    {
                        if (m_addr_mapper->is_rit_locked(flat_bank_id, row_id))
                        {
                            ...
                            if (m_is_debug)
                            {
                                std::cout << "Row " << row_id << " is already swapped with row "
                                          << prev_swapped_row << " in the current epoch."
                                          << std::endl;
                            ...
                            if (m_addr_mapper->is_rit_full(flat_bank_id))
                            {
                                ...
                                auto unswap_pair = m_addr_mapper->get_unswap_pair(
                                    flat_bank_id, m_hot_row_tracker[flat_bank_id]);
                                if (m_is_debug)
                                {
                                    std::cout << "RIT is full." << std::endl;
                                    std::cout << "Unswapping row " << unswap_pair.first
                                              << " with row " << unswap_pair.second << std::endl;
                                }
                                // unswap the pair
                                issue_swap(req_it, unswap_pair.first, unswap_pair.second);
                                // remove the pair from the rit
                                m_addr_mapper->rit_remove_entry(flat_bank_id, unswap_pair.first,
                                                                unswap_pair.second);
                            ...
                            if (m_is_debug)
                            {
                                std::cout << "Swapping row " << row_id << " with row " << dst_row0
                                          << std::endl;
                                std::cout << "Swapping row " << prev_swapped_row << " with row "
                                          << dst_row1 << std::endl;
                            }
                            ...
                            m_addr_mapper->rit_insert_entry(flat_bank_id, prev_swapped_row,
                                                            dst_row0);
                        ...
                        else
                        {
                            ...
                            {
                                std::cout << "Row " << row_id << " is already swapped with row "
                                          << prev_swapped_row << " in the previous epochs."
                                          << std::endl;
                                std::cout << "We need to unswap and reswap the row." << std::endl;
                                std::cout << "Unswapping row " << row_id << " with row "
                                          << prev_swapped_row << std::endl;
                                std::cout << "Swapping row " << row_id << " with row " << dst_row
                                          << std::endl;
                            }
                    ...
                    else
                    {
                        ...
                        if (m_addr_mapper->is_rit_full(flat_bank_id))
                        {
                            ...
                            {
                                std::cout << "RIT is full." << std::endl;
                                std::cout << "Unswapping row " << unswap_pair.first << " with row "
                                          << unswap_pair.second << std::endl;
                            }
                            // unswap the pair
                            issue_swap(req_it, unswap_pair.first, unswap_pair.second);
                            // remove the pair from the rit
    ...
    int get_rand_row(int bank_id, int row_id)
    {
        ...
        while (dst_row == -1)
        {
            ...
            if (m_hot_row_tracker[bank_id].find(rand_row) == m_hot_row_tracker[bank_id].end() &&
                m_addr_mapper->check_rit(bank_id, rand_row) == -1 && rand_row != row_id)
            {
                dst_row = rand_row;
            }
        }
