...
void SimpleO3::init()
{
    ...
    // Create the LLC
    m_llc = new SimpleO3LLC(llc_latency, llc_capacity_per_core * m_num_cores, llc_linesize_bytes,
    ...
    {
        SimpleO3Core* core = new SimpleO3Core(id, ipc, depth, m_num_expected_insts, trace_list[id],
                                              m_translation, m_llc);
...
void SimpleO3::connect_memory_system(IMemorySystem* memory_system)
{
    m_llc->connect_memory_system(memory_system);
};

int SimpleO3::get_num_cores()
{
    return m_num_cores;
}
