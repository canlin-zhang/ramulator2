...
template <typename elem_t> struct HistoryEntry
{
    elem_t entry;
    uint64_t timestamp;
};

template <typename elem_t> class IBloomFilter
{
  public:
...
template <typename elem_t, typename ctr_t> class CountingBloomFilter : public IBloomFilter<elem_t>
{
  public:
    CountingBloomFilter(int num_counters, int ctr_thresh, bool saturate,
                        std::vector<bloom_hash_fn>& hash_functions)
...
template <typename elem_t, class T> class UnifiedBloomFilter : public IBloomFilter<elem_t>
{
  public:
    UnifiedBloomFilter(std::vector<T*>& filters, int len_epoch, BHO3LLC* llc) : m_filters(filters)
    {
        static_assert(std::is_base_of<IBloomFilter<elem_t>, T>::value,
                      "Template T must be a subclass of IBloomFilter");
...
template <typename elem_t> class HistoryBuffer
{
  public:
