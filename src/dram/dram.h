...
class IDRAM : public Clocked<IDRAM>
{
     ...
     ***********************************************/
  public:
    int m_internal_prefetch_size = -1; // Internal prefetch (xn) size: How many columns are fetched
                                       // into the I/O? e.g., DDR4 has 8n prefetch.
  ...
  public:
    ...
    SpecLUT<Level_t> m_command_scopes{
        m_commands}; // A LUT of the scopes (i.e., at which organization level) of the DRAM commands
    SpecLUT<DRAMCommandMeta> m_command_meta{
        m_commands}; // A LUT to check which DRAM command opens a row

    SpecDef m_requests; // The definition of all requests supported
    SpecLUT<Command_t> m_request_translations{
        m_requests}; // A LUT of the final DRAM commands needed by every request

    // TODO: make this a priority queue
    std::vector<FutureAction>
        m_future_actions; // A vector of requests that requires future state changes
  ...
  public:
    ...
    Clk_t m_read_latency =
        -1; // Number of cycles needed between issuing RD command and receiving data.
  ...
  public:
    ...
    std::vector<PowerStats> m_power_stats; // The power stats and counters PER channel PER rank
                                           // (ch0rank0, ch0rank1... ch1rank0,...)
    SpecDef m_voltages;                    // The names of the voltage constraints
    SpecLUT<double> m_voltage_vals{
        m_voltages};    // The LUT of the values for each voltage constraints
    SpecDef m_currents; // The names of the current constraints
    SpecLUT<double> m_current_vals{
        m_currents}; // The LUT of the values for each current constraints
  ...
  public:
     ...
     * @details
     * Provide a universal interface to let the host change the DRAM configurations on the fly
     * (e.g., set refresh mode), given that the host knows what it can configure with the DRAM. This
     * is slow so should NOT be called too often
