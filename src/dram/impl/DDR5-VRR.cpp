...
class DDR5VRR : public IDRAM, public Implementation
{
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
        ...
        //   nREFSBRD nRFM1 nRFM2 nRFMsb nDRFMab nDRFMsb nVRR nCS, tCK_ps
        {"DDR5_3200AN",
         {3200, 8,  24, 24, 24, 52, 75, 48, 12, 22, 2,  8,  8,  22 + 8 + 4, 8, 16, 22 + 8 + 16,
          8,    -1, -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1,         2, 625}},
        {"DDR5_3200BN",
         {3200, 8,  26, 26, 26, 52, 77, 48, 12, 24, 2,  8,  8,  24 + 8 + 4, 8, 16, 24 + 8 + 16,
          8,    -1, -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1,         2, 625}},
        {"DDR5_3200C",
         {3200, 8,  28, 28, 28, 52, 79, 48, 12, 26, 2,  8,  8,  26 + 8 + 4, 8, 16, 26 + 8 + 16,
          8,    -1, -1, -1, -1, -1, -1, 30, -1, -1, -1, -1, -1, -1,         2, 625}},
    };
    ...
    inline static constexpr ImplDef m_commands = {
        "ACT",        "PRE",        "PREA",      "PREsb",     "RD",        "WR",
        "RDA",        "WRA",        "REFab",     "REFsb",     "REFab_end", "REFsb_end",
        "RFMab",      "RFMsb",      "RFMab_end", "RFMsb_end", "DRFMab",    "DRFMsb",
        "DRFMab_end", "DRFMsb_end", "VRR",       "VRR_end",
    };

    inline static const ImplLUT m_command_scopes = LUT(
        m_commands, m_levels,
        {
            {"ACT", "row"},     {"PRE", "bank"},     {"PREA", "rank"},       {"PREsb", "bank"},
    ...
    inline static const ImplLUT m_request_translations = LUT(m_requests, m_commands,
                                                             {{"read", "RD"},
                                                              {"write", "WR"},
    ...
    inline static constexpr ImplDef m_timings = {
        ...
        "nDRFMab", "nDRFMsb",  "nVRR",      "nCS",      "tCK_ps"};
    ...
    inline static constexpr ImplDef m_cmds_counted = {"ACT", "PRE", "RD", "WR",
                                                      "REF", "RFM", "VRR"};
  ...
  public:
    ...
    {
        Node(DDR5VRR* dram, Node* parent, int level, int id)
            : DRAMNodeBase<DDR5VRR>(dram, parent, level, id){};
    };
  ...
  private:
    ...
    void set_timing_vals()
    {
        ...
        if (prac_enabled)
        {
            std::cout << "[Ramulator::DDR5-VRR] <PRAC ENABLED> Timings are updated automatically. "
                      << std::endl;
            // Table 343 JEDEC 79-5C_v1.30
            m_timing_vals("nRAS") =
                std::max((int)JEDEC_rounding_DDR5(16, tCK_ps), m_timing_vals("nRAS"));
            m_timing_vals("nRP") =
                std::max((int)JEDEC_rounding_DDR5(36, tCK_ps), m_timing_vals("nRP"));
            m_timing_vals("nRC") =
                std::max((int)JEDEC_rounding_DDR5(52, tCK_ps), m_timing_vals("nRC"));
            m_timing_vals("nRTP") =
                std::max((int)JEDEC_rounding_DDR5(5, tCK_ps), m_timing_vals("nRTP"));
            m_timing_vals("nWR") =
                std::max((int)JEDEC_rounding_DDR5(10, tCK_ps), m_timing_vals("nWR"));

            // Table 347
            m_timing_vals("nRFM1") = std::max(
                (int)((2 * m_BRC + 1) * JEDEC_rounding_DDR5(tRRFsb_TABLE[0][density_id], tCK_ps)),
                m_timing_vals("nRFM1"));
            m_timing_vals("nRFMsb") = std::max(
                (int)((2 * m_BRC + 1) * JEDEC_rounding_DDR5(tRRFsb_TABLE[1][density_id], tCK_ps)),
                m_timing_vals("nRFMsb"));
        }
...
#define V(timing) (m_timing_vals(timing))
        ...
        populate_timingcons(
            this,
            {
                ...
                {.level = "rank",
                 .preceding = {"PREA"},
                 .following = {"REFab", "RFMab", "DRFMab", "REFsb", "RFMsb", "DRFMsb"},
                 .latency = V("nRP")},
                {.level = "rank",
                 .preceding = {"RDA"},
                 .following = {"REFab", "RFMab", "DRFMab"},
                ...
                {.level = "rank",
                 .preceding = {"REFab"},
                 .following = {"ACT", "VRR", "PREA", "REFab", "RFMab", "DRFMab", "REFsb", "RFMsb",
                               "DRFMsb"},
                 .latency = V("nRFC1")},
                {.level = "rank",
                 .preceding = {"RFMab"},
                ...
                {.level = "bank",
                 .preceding = {"ACT"},
                 .following = {"ACT", "VRR", "REFsb", "RFMsb", "DRFMsb"},
                 .latency = V("nRC")},
                {.level = "bank",
                 .preceding = {"VRR"},
                 .following = {"ACT", "VRR", "REFsb", "RFMsb", "DRFMsb"},
...
#undef V
    ...
    void set_actions()
    {
        ...
        m_actions[m_levels["rank"]][m_commands["REFab_end"]] =
            Lambdas::Action::Rank::REFab_end<DDR5VRR>;
        ...
        m_actions[m_levels["bank"]][m_commands["VRR_end"]] =
            Lambdas::Action::Bank::VRR_end<DDR5VRR>;
    };
    ...
    void set_preqs()
    {
        ...
        m_preqs[m_levels["rank"]][m_commands["REFab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5VRR>;
        m_preqs[m_levels["rank"]][m_commands["RFMab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5VRR>;
        m_preqs[m_levels["rank"]][m_commands["DRFMab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5VRR>;
        m_preqs[m_levels["rank"]][m_commands["PREA"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<DDR5VRR>;

        // Same-Bank Preqs.
        m_preqs[m_levels["rank"]][m_commands["REFsb"]] =
            Lambdas::Preq::Rank::RequireSameBanksClosed<DDR5VRR>;
        ...
        m_preqs[m_levels["bank"]][m_commands["VRR"]] =
            Lambdas::Preq::Bank::RequireBankClosed<DDR5VRR>;
    ...
    void set_powers()
    {
        ...
        m_powers[m_levels["rank"]][m_commands["REFab_end"]] =
            Lambdas::Power::Rank::REFab_end<DDR5VRR>;
