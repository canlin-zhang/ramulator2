...
void LinearMapperBase_with_rit::setup(IFrontEnd* frontend, IMemorySystem* memory_system)
{
    ...
    catch (const std::out_of_range& r)
    {
        throw std::runtime_error(
            fmt::format("Organization \"row\" not found in the spec, cannot use linear mapping!"));
    }
...
{
    if (m_row_indirection_table[flat_bank_id].find(src_row) !=
        m_row_indirection_table[flat_bank_id].end())
    {
...
std::pair<int, int>
LinearMapperBase_with_rit::get_unswap_pair(int flat_bank_id,
                                           const std::unordered_map<int, int>& exclusion_list)
{
    std::pair<int, int> unswap_pair;
    for (auto& entry : m_row_indirection_table[flat_bank_id])
    {
        if (!entry.second.lock && exclusion_list.find(entry.first) == exclusion_list.end() &&
            exclusion_list.find(entry.second.dst_row) == exclusion_list.end())
        {
...
void LinearMapperBase_with_rit::dump_rit(int flat_bank_id)
{
    std::cout << "======================" << std::endl
              << "RIT[" << flat_bank_id
              << "].size(): " << m_row_indirection_table[flat_bank_id].size() << std::endl;

    for (auto entry : m_row_indirection_table[flat_bank_id])
    {
        std::cout << entry.first << " -> " << entry.second.dst_row << "\t"
                  << (entry.second.lock ? "locked" : "unlocked") << std::endl;
    }
    std::cout << "======================" << std::endl;
}
...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, ChRaBaRoCo_with_rit, "ChRaBaRoCo_with_rit",
...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, RoBaRaCoCh_with_rit, "RoBaRaCoCh_with_rit",
...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, MOP4CLXOR_with_rit, "MOP4CLXOR_with_rit",
                                      "Applies a MOP4CLXOR mapping to the address.");
  ...
  public:
    ...
    void apply(Request& req) override
    {
        ...
        req.addr_vec[m_col_bits_idx] += slice_lower_bits(addr, m_addr_bits[m_col_bits_idx] - 2)
                                        << 2;
        ...
        for (int lvl = 0; lvl < m_col_bits_idx; lvl++)
        {
            ...
            {
                int mask =
                    (req.addr_vec[m_col_bits_idx] >> row_xor_index) & ((1 << m_addr_bits[lvl]) - 1);
