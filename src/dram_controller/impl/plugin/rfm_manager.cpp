...
class RFMManager : public IControllerPlugin, public Implementation
{
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        {
            std::cout << "[Ramulator::RFMManager] [CRITICAL ERROR] DRAM Device does not support "
                         "request: rfm"
                      << std::endl;
            exit(0);
        }
        ...
        m_num_banks_per_bankgroup =
            m_dram->get_level_size("bankgroup") < 0 ? 0 : m_dram->get_level_size("bank");
        m_num_banks_per_rank =
            m_dram->get_level_size("bankgroup") < 0
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        {
            std::cout << "[Ramulator::RFMManager] [CRITICAL ERROR] Could not send request: rfm"
                      << std::endl;
            exit(0);
        }
        s_rfm_counter++;
