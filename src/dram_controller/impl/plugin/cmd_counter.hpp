...
class CommandCounter : public IControllerPlugin, public Implementation
{
  ...
  public:
    void init() override
    {
        m_commands_to_count = param<std::vector<std::string>>("commands_to_count")
                                  .desc("A list of commands to be counted")
                                  .required();
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        for (const auto& command_name : m_commands_to_count)
        {
            ...
            {
                throw ConfigurationError("Command {} does not exist in the DRAM standard {}!",
                                         command_name, m_dram->get_name());
            }
