...
class Hydra : public IControllerPlugin, public Implementation
{
  ...
  private:
    ...
    // each entry has a group counter and a flag indicating if the group counter has beed
    // initialized the row group id uses the most significant bits of the row id
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        {
            throw ConfigurationError("Hydra is not compatible with the DRAM implementation that "
                                     "does not have Victim-Row-Refresh (VRR) command!");
        }
        ...
        m_num_banks_per_rank =
            m_dram->get_level_size("bankgroup") == -1
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        if (request_found)
        {
                ...
                m_dram->m_command_scopes(req_it->command) == m_row_level)
            {
                ...
                uint gct_index =
                    row_id >> (m_row_address_bits - m_gct_index_bits); // get most significant bits
                uint rcc_index =
                    row_id & ((1 << m_rcc_index_bits) - 1); // get least significant bits
                uint rcc_tag = row_id >> (m_row_address_bits -
                                          m_rcc_tag_row_bits)   // most significant bits of row_id
                               | bank_id << m_rcc_tag_row_bits; // bank_id

                if (m_is_debug)
                {
                    std::cout << "----------------------------------" << std::endl
                              << "Hydra: Activation cmd (" << m_clk << ") " << flat_bank_id << ","
                              << gct_index << "," << row_id << std::endl
                              << "        flat_bank_id: " << std::setw(6) << flat_bank_id << " - "
                              << std::bitset<5>(flat_bank_id) << std::endl
                              << "        rank_id:      " << std::setw(6) << rank_id << " - "
                              << std::bitset<1>(rank_id) << std::endl
                              << "        bank_id:      " << std::setw(6) << bank_id << " -  "
                              << std::bitset<4>(bank_id) << std::endl
                              << "        row_id:       " << std::setw(6) << row_id << " -      "
                              << std::bitset<16>(row_id) << std::endl
                              << "        gct_index:    " << std::setw(6) << gct_index << " -      "
                              << std::bitset<9>(gct_index) << std::endl
                              << "        rcc_index:    " << std::setw(6) << rcc_index
                              << " -              " << std::bitset<8>(rcc_index) << std::endl
                              << "        rcc_tag:      " << std::setw(6) << rcc_tag << " -  "
                              << std::bitset<12>(rcc_tag) << std::endl;
                }
                ...
                if (row_id < m_total_rct_row_size)
                {
                    ...
                    if (rct_count_table[flat_bank_id].find(row_id) ==
                        rct_count_table[flat_bank_id].end())
                    {
                    ...
                    if (m_is_debug)
                    {
                        ...
                        std::cout << "Hydra: RCT_count_table incremented ("
                                  << rct_count_table[flat_bank_id][row_id] << ")" << std::endl;
                    }
                    ...
                    if (rct_count_table[flat_bank_id][row_id] >= m_tracking_threshold)
                    {
                        if (m_is_debug)
                        {
                            std::cout << "Hydra: RCT_count_table above threshold, issue VRR, reset "
                                         "counter"
                                      << std::endl;
                        }
                        // issue VRR
                    ...
                    else
                    {
                        if (m_is_debug)
                        {
                            std::cout << "Hydra: RCT_count_table below threshold, do nothing"
                                      << std::endl;
                        }
                    }
                    return;
                ...
                if (group_count_table[flat_bank_id].find(gct_index) ==
                    group_count_table[flat_bank_id].end())
                {
                ...
                if (group_count_table[flat_bank_id][gct_index].group_count >= m_group_threshold)
                {
                    ...
                    if (!group_count_table[flat_bank_id][gct_index].initialized)
                    {
                        ...
                        for (int i = 0; i < m_group_rct_cl_size; i++)
                        {
                            ...
                            std::pair<Addr_t, Addr_t> init_row_col_id =
                                generate_row_col_id(row_group_start_row_id + i * m_rct_per_cl);
                            ...
                            if (m_is_debug)
                            {
                                std::cout << "Hydra: Group initializing, generating write request "
                                             "to DRAM for RCT"
                                          << std::endl
                                          << "        rct_bank: " << flat_bank_id << std::endl
                                          << "        rct_row:  " << rct_init_addr_vec[m_row_level]
                                          << std::endl
                                          << "        rct_col:  " << rct_init_addr_vec[m_col_level]
                                          << std::endl;
                            }
                        }
                    }
                    ...
                    if (m_is_debug)
                    {
                        std::cout << "Hydra: Checking RCC[" << rank_id << "][" << rcc_index
                                  << "].size() = " << row_count_cache[rank_id][rcc_index].size()
                                  << std::endl;
                        for (auto it = row_count_cache[rank_id][rcc_index].begin();
                             it != row_count_cache[rank_id][rcc_index].end(); it++)
                        {
                            std::cout << "        tag: " << std::setw(6) << it->first
                                      << " counter: " << it->second << std::endl;
                        }
                    }

                    // check rcc
                    s_rcc_check++;
                    if (row_count_cache[rank_id][rcc_index].find(rcc_tag) ==
                        row_count_cache[rank_id][rcc_index].end())
                    {
                        s_num_rcc_miss++;
                        if (m_is_debug)
                        {
                        ...
                        if (row_count_cache[rank_id][rcc_index].size() == 16)
                        {
                            ...
                            if (m_is_debug)
                            {
                                std::cout << "Hydra: RCC full, evicting " << tag_to_evict
                                          << std::endl;
                            }
                            ...
                            int evicted_row_id = (tag_to_evict & ((1 << m_rcc_tag_row_bits) - 1))
                                                     << m_rcc_index_bits |
                                                 rcc_index;
                            int evicted_bank_id = tag_to_evict >> m_rcc_tag_row_bits;
                            std::pair<Addr_t, Addr_t> evicted_row_col_id =
                                generate_row_col_id(evicted_row_id);
                            evicted_entry_addr_vec[m_bank_group_level] =
                                evicted_bank_id / m_dram->get_level_size("bank");
                            evicted_entry_addr_vec[m_bank_level] =
                                evicted_bank_id % m_dram->get_level_size("bank");
                            ...
                            {
                                std::cout
                                    << "Hydra: Generating write request to DRAM for evicted entry"
                                    << std::endl
                                    << "        evicted_row_id:  " << std::setw(6) << evicted_row_id
                                    << " -     " << std::bitset<16>(evicted_row_id) << std::endl
                                    << "        evicted_bank_id: " << std::setw(6)
                                    << evicted_bank_id << " - " << std::bitset<4>(evicted_bank_id)
                                    << std::endl
                                    << "        evicted_tag:     " << std::setw(6) << tag_to_evict
                                    << " - " << std::bitset<12>(tag_to_evict) << std::endl
                                    << "        rct_bank:        " << std::setw(6)
                                    << evicted_bank_id << std::endl
                                    << "        rct_row:         " << std::setw(6)
                                    << evicted_entry_addr_vec[m_row_level] << std::endl
                                    << "        rct_col:         " << std::setw(6)
                                    << evicted_entry_addr_vec[m_col_level] << std::endl;
                            }
                        }
                        else
                        {
                        ...
                        std::pair<Addr_t, Addr_t> row_col_id =
                            generate_row_col_id(rct_read_addr_vec[m_row_level]);
                        ...
                        if (m_is_debug)
                        {
                            std::cout << "Hydra: Generating read request to DRAM for RCT"
                                      << std::endl
                                      << "        rct_bank: " << flat_bank_id << std::endl
                                      << "        rct_row:  " << rct_read_addr_vec[m_row_level]
                                      << std::endl
                                      << "        rct_col:  " << rct_read_addr_vec[m_col_level]
                                      << std::endl;
                    ...
                    if (m_is_debug)
                    {
                        std::cout << "Hydra: Checking RCC counter ("
                                  << row_count_cache[rank_id][rcc_index][rcc_tag] << ")"
                                  << std::endl;
                    }
                    ...
                    if (row_count_cache[rank_id][rcc_index][rcc_tag] >= m_tracking_threshold)
                    {
                        if (m_is_debug)
                        {
                            std::cout << "Hydra: RCC above threshold, issue VRR, reset counter"
                                      << std::endl;
                        }
                        // issue VRR
                        Request vrr_req(req_it->addr_vec, m_VRR_req_id);
                ...
                else
                {
                    ...
                    {
                        std::cout << "Hydra: Checking GCT" << std::endl;
                        std::cout << "Hydra: GCT below threshold ("
                                  << group_count_table[flat_bank_id][gct_index].group_count << ")"
                                  << std::endl;
