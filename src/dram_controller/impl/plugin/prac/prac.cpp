  ...
  public:
    ...
    void update_state_machine(bool request_found, const Request& req)
    {
        ...
        case ABOState::RECOVERY:
            if (request_found && (req.command == cmd_rfmab || req.command == cmd_rfmsb))
            {
    ...
    class PerBankCounters
    {
      public:
        ...
        void init_dram_params(IDRAM* dram)
        {
            CommandHandler handlers[] = {
                ...
                {std::string("ACT"),
                 std::bind(&PerBankCounters::process_act, this, std::placeholders::_1)},
                {std::string("RFMab"),
                 std::bind(&PerBankCounters::process_rfm, this, std::placeholders::_1)},
                {std::string("RFMsb"),
                 std::bind(&PerBankCounters::process_rfm, this, std::placeholders::_1)}};
            for (auto& h : handlers)
            {
      ...
      private:
        ...
        void process_rfm(const Request& req)
        {
            ...
            if (m_debug)
            {
                std::printf("[PRAC] [%d] [RFM] Row: %d Act: %u\n", m_bank_id, act_max->first,
                            m_counters[act_max->first]);
            }
