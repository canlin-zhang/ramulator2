...
template <typename T, typename = std::void_t<typename T::Node>> struct DRAMNodeBase
{
    ...
    std::vector<Clk_t>
        m_cmd_ready_clk; // The next cycle that each command can be issued again at this level
        ...
        : m_spec(spec), m_parent_node(parent), m_level(level), m_node_id(id)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    NodeType* child =
                        new NodeType(spec, static_cast<NodeType*>(this), next_level, i);
    ...
    void update_states(int command, const AddrVec_t& addr_vec, Clk_t clk)
    {
        ...
        if (m_spec->m_actions[m_level][command])
        {
            ...
            m_spec->m_actions[m_level][command](static_cast<NodeType*>(this), command, child_id,
                                                clk);
        }
    ...
    void update_powers(int command, const AddrVec_t& addr_vec, Clk_t clk)
    {
        ...
        if (m_spec->m_powers[m_level][command])
        {
            ...
            m_spec->m_powers[m_level][command](static_cast<NodeType*>(this), command, addr_vec,
                                               clk);
        }
        if (m_level == m_spec->m_command_scopes[command] || !m_child_nodes.size())
        {
    ...
    int get_preq_command(int command, const AddrVec_t& addr_vec, Clk_t m_clk)
    {
        ...
        {
            int preq_cmd = m_spec->m_preqs[m_level][command](static_cast<NodeType*>(this), command,
                                                             addr_vec, m_clk);
            if (preq_cmd != -1)
            {
    ...
    bool check_rowbuffer_hit(int command, const AddrVec_t& addr_vec, Clk_t m_clk)
    {
        ...
        if (m_spec->m_rowhits[m_level][command])
        {
            ...
            return m_spec->m_rowhits[m_level][command](static_cast<NodeType*>(this), command,
                                                       child_id, m_clk);
        }
    ...
    bool check_node_open(int command, const AddrVec_t& addr_vec, Clk_t m_clk)
    {
        ...
        if (m_spec->m_rowopens[m_level][command])
            ...
            return m_spec->m_rowopens[m_level][command](static_cast<NodeType*>(this), command,
                                                        child_id, m_clk);
...
template <class T>
using PreqFunc_t =
    std::function<int(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)>;
template <class T>
using RowhitFunc_t = std::function<bool(typename T::Node* node, int cmd, int target_id, Clk_t clk)>;
template <class T>
using RowopenFunc_t =
    std::function<bool(typename T::Node* node, int cmd, int target_id, Clk_t clk)>;
template <class T>
using PowerFunc_t =
    std::function<void(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)>;

template <typename T> using FuncMatrix = std::vector<std::vector<T>>;
