...
class HBM3 : public IDRAM, public Implementation
{
  ...
  public:
     ...
     *             Requests & Commands
     ***********************************************/
    inline static constexpr ImplDef m_commands = {"ACT", "PRE",   "PREA",  "RD",    "WR",   "RDA",
  ...
  public:
    ...
    {
        Node(HBM3* dram, Node* parent, int level, int id)
            : DRAMNodeBase<HBM3>(dram, parent, level, id){};
    };
...
#define V(timing) (m_timing_vals(timing))
        populate_timingcons(
            this,
            {
                ...
                {.level = "channel",
                 .preceding = {"ACT"},
                 .following = {"ACT", "PRE", "PREA", "REFab", "REFsb", "RFMab", "RFMsb"},
                 .latency = 2},
...
#undef V
    ...
    void set_preqs()
    {
        ...
        m_preqs[m_levels["channel"]][m_commands["REFab"]] =
            Lambdas::Preq::Channel::RequireAllBanksClosed<HBM3>;

        // Bank actions
        m_preqs[m_levels["bank"]][m_commands["REFsb"]] =
            Lambdas::Preq::Bank::RequireBankClosed<HBM3>;
