...
class Graphene : public IControllerPlugin, public Implementation
{
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        {
            throw ConfigurationError("Graphene is not compatible with the DRAM implementation that "
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        if (m_clk % m_reset_period_clk == 0)
        {
            ...
            for (int i = 0; i < m_num_banks_per_rank * m_num_ranks; i++)
            {
                for (auto it = m_activation_count_table[i].begin();
                     it != m_activation_count_table[i].end(); it++)
                    it->second = 0;
        ...
        if (request_found)
        {
                ...
                m_dram->m_command_scopes(req_it->command) == m_row_level)
            {
                ...
                if (m_is_debug)
                {
                    ...
                    std::cout << "  └  " << "bank_group: " << req_it->addr_vec[m_rank_level + 1]
                              << std::endl;
                    std::cout << "  └  " << "bank: " << req_it->addr_vec[m_bank_level] << std::endl;
                    std::cout << "  └  " << "index: " << flat_bank_id << std::endl;
                }

                if (m_activation_count_table[flat_bank_id].find(row_id) ==
                    m_activation_count_table[flat_bank_id].end())
                {
                    ...
                    for (auto it = m_activation_count_table[flat_bank_id].begin();
                         it != m_activation_count_table[flat_bank_id].end(); it++)
                    {
                        if (m_is_debug)
                            std::cout << "  └  " << "checking row " << it->first << " with count "
                                      << it->second << std::endl;
                    ...
                    if (found)
                    {
                        ...
                        if (m_is_debug)
                        {
                            ...
                            std::cout << "Removing row " << to_remove << " from table "
                                      << flat_bank_id << std::endl;
                            // print the row that is being added
                            std::cout << "Adding row " << row_id << " to table " << flat_bank_id
                                      << std::endl;
                            std::cout << "  └  "
                                      << "spillover counter: " << m_spillover_counter[flat_bank_id]
                                      << std::endl;
                        }
                ...
                else
                {
                    ...
                    if (m_is_debug)
                    {
                        std::cout << "Row " << row_id << " in table[" << flat_bank_id << "]"
                                  << std::endl;
                        std::cout << "  └  " << "threshold: " << m_activation_threshold
                                  << std::endl;
                        std::cout << "  └  "
                                  << "count: " << m_activation_count_table[flat_bank_id][row_id]
                                  << std::endl;
                    }
                    ...
                    if (m_activation_count_table[flat_bank_id][row_id] >= m_activation_threshold)
                    {
                        ...
                        m_activation_count_table[flat_bank_id][row_id] =
                            m_spillover_counter[flat_bank_id];
                    }
                }
            }
        }
