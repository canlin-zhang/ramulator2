...
namespace Bank
{
template <class T> int get_flat_rank_id(typename T::Node* node)
{
    int channel_id = -1;
    int rank_id = -1;
...
template <class T> void debug(typename T::Node* node, std::string msg, Clk_t clk)
{
    if (node->m_spec->m_power_debug)
    {
        std::cout << "[Power] Rank" << Bank::get_flat_rank_id<T>(node) << " Bank" << node->m_node_id
                  << " " << msg << " @ " << clk << std::endl;
    }
}

template <class T> void ACT(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing ACT counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("ACT")]++;
}

template <class T> void PRE(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing PRE counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("PRE")]++;
}

template <class T> void RD(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing RD counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("RD")]++;
}

template <class T> void WR(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing WR counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("WR")]++;
}

template <class T> void VRR(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing VRR counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("VRR")]++;
}

template <class T> void RVRR(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    Bank::debug<T>(node, "Incrementing RVRR counter.", clk);
    node->m_spec->m_power_stats[Bank::get_flat_rank_id<T>(node)]
        .cmd_counters[T::m_cmds_counted("RVRR")]++;
}
} // namespace Bank
...
template <class T> void debug(typename T::Node* node, std::string msg, Clk_t clk)
{
    if (node->m_spec->m_power_debug)
    {
        std::cout << "[Power] Rank" << Rank::get_flat_rank_id<T>(node) << " " << msg << " @ " << clk
                  << std::endl;
    }
}

template <class T> int get_open_bank_count(typename T::Node* node)
{
    int bank_count = 0;
    if constexpr (T::m_levels["bank"] - T::m_levels["rank"] == 1)
    {
...
template <class T> int get_refreshing_bank_count(typename T::Node* node)
{
    int bank_count = 0;
    if constexpr (T::m_levels["bank"] - T::m_levels["rank"] == 1)
    {
...
template <class T> void ACT(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    bool is_rank_idle =
        get_open_bank_count<T>(node) == 0 && get_refreshing_bank_count<T>(node) == 0;
    ...
    if (is_rank_idle)
    {
        ...
        std::string msg =
            "Rank is idle. idle_cycles: " + std::to_string(cur_power_stats.idle_cycles) +
            "    active_start_cycle: " + std::to_string(cur_power_stats.active_start_cycle);
...
template <class T> void PRE(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
    bool is_rank_going_idle =
        get_open_bank_count<T>(node) == 1 &&
        get_refreshing_bank_count<T>(node) == 0; // TODO: AND this PRE is targetting the active bank
    ...
    if (is_rank_going_idle)
    {
        ...
        std::string msg =
            "Rank is going idle. active_cycles: " + std::to_string(cur_power_stats.active_cycles) +
            "    idle_start_cycle: " + std::to_string(cur_power_stats.idle_start_cycle);
        Rank::debug<T>(node, msg, clk);
        cur_power_stats.cur_power_state = PowerStats::PowerState::IDLE;
    }
}

template <class T> void PREA(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    assert(get_refreshing_bank_count<T>(node) == 0 &&
           "PREA should not be called when there are refreshing banks");
    ...
    if (!is_rank_idle)
    {
        ...
        cur_power_stats.idle_start_cycle = clk;
        std::string msg =
            "Rank is not idle. active_cycles: " + std::to_string(cur_power_stats.active_cycles) +
            "    idle_start_cycle: " + std::to_string(cur_power_stats.idle_start_cycle);
        Rank::debug<T>(node, msg, clk);
        cur_power_stats.cur_power_state = PowerStats::PowerState::IDLE;
    }
...
template <class T> void REFab(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
...
void REFab_end(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    cur_power_stats.idle_start_cycle = clk;
    std::string msg =
        "Refresh ends. idle_start_cycle: " + std::to_string(cur_power_stats.idle_start_cycle);
    Rank::debug<T>(node, msg, clk);
    cur_power_stats.cur_power_state = PowerStats::PowerState::IDLE;
}
...
void VRR_end(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
    bool is_rank_going_idle =
        get_open_bank_count<T>(node) == 0 && get_refreshing_bank_count<T>(node) == 1;

    if (is_rank_going_idle)
    {
        cur_power_stats.active_cycles += clk - cur_power_stats.active_start_cycle;
        cur_power_stats.idle_start_cycle = clk;
        std::string msg = "Rank is going idle. idle_start_cycle: " +
                          std::to_string(cur_power_stats.idle_start_cycle) +
                          "    active_cycles: " + std::to_string(cur_power_stats.active_cycles);
        Rank::debug<T>(node, msg, clk);
        cur_power_stats.cur_power_state = PowerStats::PowerState::IDLE;
    }
}

template <class T> void RFMsb(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
...
void RFMsb_end(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    ...
    bool is_rank_going_idle =
        get_open_bank_count<T>(node) == 0 && get_refreshing_bank_count<T>(node) == num_bankgroups;
...
template <class T> void PREsb(typename T::Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
{
    auto& cur_power_stats = node->m_spec->m_power_stats[Rank::get_flat_rank_id<T>(node)];
