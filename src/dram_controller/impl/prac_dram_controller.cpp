...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(IBHDRAMController, PRACDRAMController, "PRACDRAMController",
                                      "PRAC DRAM controller.")
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        void PRACDRAMController::setup(IFrontEnd * frontend, IMemorySystem * memory_system)
        {
            m_llc = static_cast<BHO3*>(frontend)->get_llc();
            m_dram = memory_system->get_ifce<IDRAM>();
            m_rank_addr_idx = m_dram->m_levels("rank");
            ...
            void tick() override
            {
                ...
                bool is_recovery_starting =
                    m_prac->next_recovery_cycle() - m_clk <= m_dram->m_timing_vals("nRP") + 5;
            ...
            if (request_found)
            {
                ...
                // If we are issuing the last command, set depart clock cycle and move the request
          ...
          private:
             ...
             * If so, it finishes this request by calling its callback and poping it from the
             * pending queue.
             */
            void serve_completed_reads()
            {
                ...
                void PRACDRAMController::set_write_mode()
                {
                    if (!m_is_write_mode)
                    {
                        if ((m_write_buffer.size() >
                ...
                bool PRACDRAMController::schedule_request(ReqBuffer::iterator & req_it,
                                                          ReqBuffer * &req_buffer)
                {
                    ...
                    {
                        bool fits =
                            m_clk + m_prac->min_cycles_with_preall(req_it) < next_recovery_clk;
                    ...
                    // 2.2    If no requests can be scheduled from the act buffer, check the rest of
                    // the buffers
                    if (!request_found)
                    {
                        ...
                        if (m_prac_buffer.size() != 0)
                        {
                            ...
                            bool early_issue =
                                is_rfm && is_pre_rec; // Prevent controller from issuing RFMab
                                                      // before recovery starts
                            request_found = !early_issue &&
                        ...
                        // 2.2.2    We then check the priority buffer to prioritize e.g.,
                        ...
                        if (m_priority_buffer.size() != 0)
                        {
                            ...
                            bool fits =
                                m_clk + m_prac->min_cycles_with_preall(req_it) < next_recovery_clk;
                            request_found =
                                fits && m_dram->check_ready(req_it->command, req_it->addr_vec);
                        ...
                        // 2.2.3    If no request to be scheduled in the priority buffer, check the
                        ...
                        if (!request_found)
                        {
                                ...
                                req_it != buffer.end())
                            {
                                ...
                                bool is_rfm = req_it->command == m_dram->m_commands("RFMab");
                                bool is_pre_rec =
                                    m_prac->get_state() == IPRAC::ABOState::PRE_RECOVERY;
                                bool early_issue =
                                    is_rfm && is_pre_rec; // Prevent controller from issuing RFMab
                                                          // before recovery starts
                                request_found =
                            ...
                            if (m_priority_buffer.size() != 0)
                            {
                                ...
                                bool fits = m_clk + m_prac->min_cycles_with_preall(req_it) <
                                            next_recovery_clk;
                                request_found =
                                    fits && m_dram->check_ready(req_it->command, req_it->addr_vec);
                                if (!request_found & m_priority_buffer.size() != 0)
                            ...
                            // 2.2.3    If no request to be scheduled in the priority buffer, check
                            ...
                            if (!request_found)
                            {
                                    ...
                                    req_it != buffer.end())
                                {
                                    bool fits = m_clk + m_prac->min_cycles_with_preall(req_it) <
                                                next_recovery_clk;
                                    request_found = fits && m_dram->check_ready(req_it->command,
                                                                                req_it->addr_vec);
                                    req_buffer = &buffer;
                                }
                            }
