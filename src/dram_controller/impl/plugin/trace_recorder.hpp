  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override
    {
        ...
        auto sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
            fmt::format("{}.ch{}", m_trace_path.string(), m_ctrl->m_channel_id), true);
        m_tracer = std::make_shared<spdlog::logger>(
            fmt::format("trace_recorder_ch{}", m_ctrl->m_channel_id), sink);
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        if (request_found)
        {
            m_tracer->trace("{}, {}, {}", m_clk, m_dram->m_commands(req_it->command),
