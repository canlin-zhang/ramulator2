...
void BHO3::init()
{
    ...
    std::vector<std::string> trace_list =
        param<std::vector<std::string>>("traces").desc("A list of traces.").required();
    std::vector<std::string> no_wait_trace_list =
        param<std::vector<std::string>>("no_wait_traces")
            .desc("Traces that do not block program termination.")
            .default_val(empty_trace);
    ...
    int depth = param<int>("inst_window_depth")
                    .desc("Instruction window size of the SimpleO3 core.")
                    .default_val(128);
    ...
    llc_serialize =
        param<bool>("llc_serialize").desc("Whether to serialize the LLC.").default_val(false);
    llc_serialization_filename = param<std::string>("llc_serialization_filename")
                                     .desc("Filename to serialize the LLC.")
                                     .default_val("llc_serialization");
    llc_deserialize =
        param<bool>("llc_deserialize").desc("Whether to deserialize the LLC.").default_val(false);
    llc_deserialization_filename = param<std::string>("llc_deserialization_filename")
                                       .desc("Filename to deserialize the LLC.")
                                       .default_val("llc_serialization");
    ...
    m_num_expected_insts = param<int>("num_expected_insts")
                               ...
                               .required();
    m_num_max_cycles = param<uint64_t>("num_max_cycles")
                           .desc("Number of cycles the frontend is allowed to execute.")
    ...
    // Create the LLC
    m_llc = new BHO3LLC(llc_latency, llc_capacity_per_core * m_num_cores, llc_linesize_bytes,
                        llc_associativity, llc_num_mshr_per_core * m_num_cores, m_num_cores);
    if (llc_deserialize)
    {
    ...
    for (int id = 0; id < m_num_cores; id++)
    {
        ...
        BHO3Core* core = new BHO3Core(id, ipc, depth, m_num_expected_insts, m_num_max_cycles,
                                      active_list[active_id], cur_translate, m_llc,
                                      lat_hist_sensitivity, lat_dump_path, is_attacker);
    ...
    for (int core_id = 0; core_id < m_cores.size(); core_id++)
    {
        ...
        register_stat(m_cores[core_id]->s_mem_requests_issued)
            .name("memory_requests_recorded_core_{}", core_id);
    }
}
