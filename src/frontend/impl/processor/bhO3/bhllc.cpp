...
BHO3LLC::BHO3LLC(int latency, int size_bytes, int linesize_bytes, int associativity, int num_mshrs,
                 int num_cores)
...
bool BHO3LLC::send(Request& req)
{
    ...
    if (auto line_it = check_set_hit(set, req.addr); line_it != set.end())
    {
        ...
        DEBUG_LOG(DBHO3LLC, m_logger,
                  "[Clk={}] Request Source: {}, Type: {}, Addr: {}, Index: {}, Tag: {}. Hit, will "
                  "finish at Clk={}",
                  m_clk, req.source_id, req.type_id, req.addr, get_index(req.addr),
                  get_tag(req.addr), m_clk, m_clk + m_latency);
...
{
    auto it = std::find_if(m_mshrs.begin(), m_mshrs.end(), [&req, this](MSHREntry_t mshr_entry)
...
BHO3LLC::CacheSet_t::iterator BHO3LLC::allocate_line(CacheSet_t& set, Addr_t addr)
{
    ...
    if (need_eviction(set, addr))
    {
        ...
        auto victim =
            std::find_if(set.begin(), set.end(), [this](Line line) { return line.ready; });
        if (victim == set.end())
...
void BHO3LLC::evict_line(CacheSet_t& set, CacheSet_t::iterator victim_it)
{
    ...
    if (victim_it->dirty)
    {
        ...
        DEBUG_LOG(DBHO3LLC, m_logger, "Writeback Request will be issued at Clk={}.",
                  m_clk + m_latency);
    }
...
void BHO3LLC::serialize(std::string serialization_filename)
{
    ...
    for (auto it1 = m_cache_sets.begin(); it1 != m_cache_sets.end(); it1++)
    {
        ...
        {
            serialization_file << it1->first << "," << it2->addr << "," << it2->tag << ","
                               << it2->dirty << std::endl;
        }
    }
...
void BHO3LLC::dump_llc()
{
    ...
    for (auto it1 = m_cache_sets.begin(); it1 != m_cache_sets.end(); it1++)
    {
        for (auto it2 = it1->second.begin(); it2 != it1->second.end(); it2++)
        {
            std::cout << it1->first << "," << it2->addr << "," << it2->tag << "," << it2->dirty
                      << "," << it2->ready << std::endl;
        }
    }
}
