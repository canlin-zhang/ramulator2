...
namespace Bank
{
template <class T> void ACT(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    node->m_state = T::m_states["Opened"];
    node->m_row_state[target_id] = T::m_states["Opened"];
};

template <class T> void PRE(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    node->m_state = T::m_states["Closed"];
    node->m_row_state.clear();
};

template <class T> void VRR(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    node->m_state = T::m_states["Refreshing"];
};

template <class T> void VRR_end(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    node->m_state = T::m_states["Closed"];
};
...
namespace BankGroup
{
template <class T> void PREsb(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
...
template <class T> void REFsb(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    for (auto bank : node->m_child_nodes)
    {
        if (bank->m_node_id == target_id)
...
template <class T> void REFsb_end(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    for (auto bank : node->m_child_nodes)
    {
        if (bank->m_node_id == target_id)
...
namespace Rank
{
template <class T> void PREab(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
...
template <class T> void REFab(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    for (auto bg : node->m_child_nodes)
    {
        for (auto bank : bg->m_child_nodes)
...
template <class T> void REFab_end(typename T::Node* node, int cmd, int target_id, Clk_t clk)
{
    for (auto bg : node->m_child_nodes)
    {
        for (auto bank : bg->m_child_nodes)
