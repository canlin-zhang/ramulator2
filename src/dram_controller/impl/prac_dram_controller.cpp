...
void PRACDRAMController::setup(IFrontEnd* frontend, IMemorySystem* memory_system)
{
    ...
    if (request_found)
    {
        ...
        if (request_found)
        {
            ...
            // If we are issuing the last command, set depart clock cycle and move the request to
            // the pending queue
            if (req_it->command == req_it->final_command)
            {
...
bool PRACDRAMController::schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer)
{
    bool request_found = false;
    Clk_t next_recovery_clk = m_prac->next_recovery_cycle();
    // 2.1    First, check the act buffer to serve requests that are already activating (avoid
    ...
    if (!request_found)
    {
        ...
        if (m_prac_buffer.size() != 0)
        {
            ...
            bool early_issue =
                is_rfm &&
                is_pre_rec; // Prevent controller from issuing RFMab before recovery starts
        ...
        // 2.2.3    If no request to be scheduled in the priority buffer, check the read and write
        // buffers.
        if (!request_found)
        {
            ...
            if (req_it = m_scheduler->get_best_request(buffer); req_it != buffer.end())
            {
                ...
                bool early_issue =
                    ...
                    is_pre_rec; // Prevent controller from issuing RFMab before recovery starts
                request_found =
                    !early_issue && m_dram->check_ready(req_it->command, req_it->addr_vec);
                if (!request_found & m_prac_buffer.size() != 0)
                {
                    return false;
                }
            ...
            // 2.2.3    If no request to be scheduled in the priority buffer, check the read and
            // write buffers.
            if (!request_found)
            {
                // Query the write policy to decide which buffer to serve
