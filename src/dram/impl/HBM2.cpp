...
class HBM2 : public IDRAM, public Implementation
{
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
        //   name       rate   nBL  nCL  nRCDRD  nRCDWR  nRP  nRAS  nRC  nWR  nRTPS  nRTPL  nCWL
        //   nCCDS  nCCDL  nRRDS  nRRDL  nWTRS  nWTRL  nRTW  nFAW  nRFC  nRFCSB  nREFI  nREFISB
        //   nRREFD  tCK_ps
        {"HBM2_2Gbps", {2000, 4, 7, 7, 7, 7, 17, 19, 8,   2,    3,  2, 1,
    ...
    inline static constexpr ImplDef m_requests = {"read", "write", "all-bank-refresh",
                                                  "per-bank-refresh"};
  ...
  public:
    ...
    {
        Node(HBM2* dram, Node* parent, int level, int id)
            : DRAMNodeBase<HBM2>(dram, parent, level, id){};
    };
...
#define V(timing) (m_timing_vals(timing))
        populate_timingcons(
            this,
            {
                ...
                /*** Pseudo Channel (Table 3 — Array Access Timings Counted Individually Per Pseudo
                   Channel, JESD-235C) ***/
                // RAS <-> RAS
                {.level = "pseudochannel",
                 .preceding = {"ACT"},
                ...
                {.level = "pseudochannel",
                 ...
                 .following = {"REFsb"},
                 .latency = V("nRREFD")},
                /// nRREFD is the latency between REFsb <-> ACT to *different* banks. -1 as ACT
                /// happens on its 2nd cycle
                {.level = "pseudochannel",
                 .preceding = {"REFsb"},
                 .following = {"ACT"},
                 .latency = V("nRREFD") - 1},
...
#undef V
    ...
    void set_preqs()
    {
        ...
        m_preqs[m_levels["channel"]][m_commands["REFab"]] =
            Lambdas::Preq::Channel::RequireAllBanksClosed<HBM2>;

        // Bank actions
        m_preqs[m_levels["bank"]][m_commands["REFsb"]] =
            Lambdas::Preq::Bank::RequireBankClosed<HBM2>;
