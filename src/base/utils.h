...
/**
 * @brief Convert a timing constraint in nanoseconds into number of cycles according to JEDEC DDR5
 * convention.
 *
 * @param t_ns      Timing constraint in nanoseconds
 * @param tCK_ps    Clock cycle in picoseconds
 ...
 * @return Integral_t
 */
template <typename Integral_t> Integral_t calc_log2(Integral_t val)
{
...
/**
 * @brief Slice the lest significant num_bits from addr and return these bits. The originial addr
 * value is modified.
 *
 * @tparam Integral_t
 * @param addr
 ...
 * @return Integral_t
 */
template <typename Integral_t> Integral_t slice_lower_bits(Integral_t& addr, int num_bits)
{
    static_assert(std::is_integral_v<Integral_t>,
                  "Only integral types are allowed for bitwise operations!");
