                                     ...
                                     const std::vector<std::string>& params)
{
    ...
    void Config::Details::override_configs(YAML::Node config,
                                           const std::vector<std::string>& params)
    {
            ...
            for (const auto& token : tokens)
            {
                ...
                else
                {
                    ...
                    for (auto& i : indices)
                    {
                        if (i > node.size())
                        {
                            spdlog::error("Sequence access out of bound! To append elements to a "
                                          "sequence, use "
                                          "the index as one past the end of the sequence.");
                            std::exit(-1);
                        }
                        node.reset(node[i]);
                    }
