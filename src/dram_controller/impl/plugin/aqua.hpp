    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override
    {
        ...
        if (request_found)
        {
                ...
                m_dram->m_command_scopes(req_it->command) == m_row_level)
            {
                ...
                // Check HRT
                if (m_aggressor_row_tracker[flat_bank_id].find(row_id) ==
                    m_aggressor_row_tracker[flat_bank_id].end())
                {
                    if (m_is_debug)
                    {
                    ...
                    else
                    {
                        ...
                        for (auto it = m_aggressor_row_tracker[flat_bank_id].begin();
                             it != m_aggressor_row_tracker[flat_bank_id].end(); it++)
                        {
                ...
                // if (m_is_debug) {
                //   std::cout << "==========================" << std::endl;
                //   std::cout << "HRT[" << flat_bank_id << "].size(): " <<
                //   m_hot_row_tracker[flat_bank_id].size() << std::endl; for (auto entry:
                //   m_hot_row_tracker[flat_bank_id]) {
                ...
                if (m_is_debug)
                {
                    ...
                    std::cout << "  └  "
                              << "count: " << m_aggressor_row_tracker[flat_bank_id][row_id]
                              << std::endl;
                }
                if (m_aggressor_row_tracker[flat_bank_id][row_id] % m_art_threshold == 0)
                {
                    ...
                    if (m_reverse_pointer_table[flat_bank_id].find(m_rqa_head) !=
                        m_reverse_pointer_table[flat_bank_id].end())
                    {
                        // head is valid, it is from the previous epoch (guaranteed by AQUA)
                        // evict it than issue the new migration
                        if (m_is_debug)
                        {
                            std::cout << "RQA head is valid, evicting row " << m_rqa_head
                                      << std::endl;
                        }
