...
class LPDDR5 : public IDRAM, public Implementation
{
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
        //   name         rate   nBL  nCL  nRCD  nRPab  nRPpb   nRAS  nRC   nWR  nRTP nCWL nCCD nRRD
        //   nWTRS nWTRL nFAW  nPPD  nRFCab nRFCpb nREFI nPBR2PBR nPBR2ACT nCS,  tCK_ps
        {"LPDDR5_6400", {6400, 4, 20, 15, 17, 15, 34, 30, 28, 4,  11, 4,
                         4,    5, 10, 16, 2,  -1, -1, -1, -1, -1, 2,  1250}},
    };
    ...
    inline static constexpr ImplDef m_commands = {
        "ACT-1", "ACT-2", "PRE",   "PREA",  "CASRD", "CASWR", // WCK2CK Sync
        "RD16",  "WR16",  "RD16A", "WR16A", "REFab", "REFpb", "RFMab", "RFMpb",
    };
    ...
    inline static constexpr ImplDef m_requests = {"read16", "write16", "all-bank-refresh",
                                                  "per-bank-refresh"};
    ...
    inline static constexpr ImplDef m_timings = {
        "rate", "nBL16",  "nCL",    "nRCD",  "nRPab",    "nRPpb",    "nRAS",  "nRC",
        "nWR",  "nRTP",   "nCWL",   "nCCD",  "nRRD",     "nWTRS",    "nWTRL", "nFAW",
        "nPPD", "nRFCab", "nRFCpb", "nREFI", "nPBR2PBR", "nPBR2ACT", "nCS",   "tCK_ps"};

    /************************************************
     *                 Node States
     ***********************************************/
    inline static constexpr ImplDef m_states = {//    ACT-1       ACT-2
                                                "Pre-Opened", "Opened", "Closed",
                                                "PowerUp",    "N/A",    "Refreshing"};
  ...
  public:
    struct Node : public DRAMNodeBase<LPDDR5>
    {
        ...
        Node(LPDDR5* dram, Node* parent, int level, int id)
            : DRAMNodeBase<LPDDR5>(dram, parent, level, id){};
    };
...
#define V(timing) (m_timing_vals(timing))
        populate_timingcons(
            this,
            {
                ...
                /// Data bus occupancy
                {.level = "channel",
                 .preceding = {"RD16", "RD16A"},
                 .following = {"RD16", "RD16A"},
                 .latency = V("nBL16")},
                {.level = "channel",
                 .preceding = {"WR16", "WR16A"},
                 .following = {"WR16", "WR16A"},
                 .latency = V("nBL16")},
                ...
                {.level = "rank",
                 .preceding = {"RD16", "RD16A"},
                 .following = {"WR16", "WR16A"},
                 .latency = V("nCL") + V("nBL16") + 2 - V("nCWL")},
                /// WR <-> RD, Minimum Read after Write
                {.level = "rank",
                 .preceding = {"WR16", "WR16A"},
                 .following = {"RD16", "RD16A"},
                 .latency = V("nCWL") + V("nBL16") + V("nWTRS")},
                /// CAS <-> CAS between sibling ranks, nCS (rank switching) is needed for new DQS
                {.level = "rank",
                 .preceding = {"RD16", "RD16A"},
                 .following = {"RD16", "RD16A", "WR16", "WR16A"},
                 .latency = V("nBL16") + V("nCS"),
                 .is_sibling = true},
                {.level = "rank",
                 .preceding = {"WR16", "WR16A"},
                 .following = {"RD16", "RD16A"},
                 .latency = V("nCL") + V("nBL16") + V("nCS") - V("nCWL"),
                 .is_sibling = true},
                /// CAS <-> PREab
                {.level = "rank",
                 .preceding = {"RD16"},
                ...
                {.level = "rank",
                 .preceding = {"RD16A"},
                 .following = {"REFab"},
                 .latency = V("nRPpb") + V("nRTP")},
                {.level = "rank",
                 .preceding = {"WR16A"},
                 .following = {"REFab"},
                 .latency = V("nCWL") + V("nBL16") + V("nWR") + V("nRPpb")},
                {.level = "rank",
                 .preceding = {"REFab"},
                 .following = {"REFab", "ACT-1", "REFpb"},
                 .latency = V("nRFCab")},
                {.level = "rank",
                 .preceding = {"ACT-1"},
                 .following = {"REFpb"},
                ...
                {.level = "bankgroup",
                 .preceding = {"WR16", "WR16A"},
                 .following = {"RD16", "RD16A"},
                 .latency = V("nCWL") + V("nBL16") + V("nWTRL")},
                /// RAS <-> RAS
                {.level = "bankgroup",
                 .preceding = {"ACT-1"},
                 .following = {"ACT-1"},
...
#undef V
    ...
    void set_preqs()
    {
        ...
        m_preqs[m_levels["rank"]][m_commands["REFab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<LPDDR5>;
        m_preqs[m_levels["rank"]][m_commands["RFMab"]] =
            Lambdas::Preq::Rank::RequireAllBanksClosed<LPDDR5>;

        m_preqs[m_levels["rank"]][m_commands["REFpb"]] =
            [this](Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
        {
            for (auto bg : node->m_child_nodes)
            {
                for (auto bank : bg->m_child_nodes)
                {
                    ...
                    if (flat_bankid == addr_vec[LPDDR5::m_levels["bank"]] ||
                        flat_bankid == addr_vec[LPDDR5::m_levels["bank"]] + 8)
                    {
                        switch (node->m_state)
                        {
        ...
        m_preqs[m_levels["rank"]][m_commands["RFMpb"]] =
            m_preqs[m_levels["rank"]][m_commands["REFpb"]];

        // Bank Preqs
        m_preqs[m_levels["bank"]][m_commands["RD16"]] =
            [](Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
        {
            switch (node->m_state)
            {
        ...
        m_preqs[m_levels["bank"]][m_commands["WR16"]] =
            [](Node* node, int cmd, const AddrVec_t& addr_vec, Clk_t clk)
        {
            switch (node->m_state)
            {
