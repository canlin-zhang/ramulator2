...
void GenericDRAMController::setup(IFrontEnd* frontend, IMemorySystem* memory_system)
{
...
bool GenericDRAMController::send(Request& req)
{
...
void GenericDRAMController::tick()
{
    m_clk++;

    // Update statistics
    s_queue_len +=
        m_read_buffer.size() + m_write_buffer.size() + m_priority_buffer.size() + pending.size();
...
bool GenericDRAMController::is_row_hit(ReqBuffer::iterator& req)
{
    return m_dram->check_rowbuffer_hit(req->final_command, req->addr_vec);
}

bool GenericDRAMController::is_row_open(ReqBuffer::iterator& req)
{
    return m_dram->check_node_open(req->final_command, req->addr_vec);
}

void GenericDRAMController::update_request_stats(ReqBuffer::iterator& req)
{
...
void GenericDRAMController::serve_completed_reads()
{
    if (pending.size())
    {
...
void GenericDRAMController::set_write_mode()
{
    if (!m_is_write_mode)
    {
...
bool GenericDRAMController::schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer)
{
    ...
    // 2.3 If we find a request to schedule, we need to check if it will close an opened row in the
    // active buffer.
    if (request_found)
    {
...
void GenericDRAMController::finalize()
{
