 ...
 * An implementation of an interface should inherit from both its corresponding interface class and
 * this class.
 *
 */
class Implementation
{
  ...
  protected:
    ...
    std::string
        m_id; // Identifier to distinguish between multiple instances of the same implementation.

    Implementation* m_parent; // The pointer to the parent implementation object
    std::vector<Implementation*>
        m_children; // A vector of pointers to the child implementation objects.
  ...
  public:
    Implementation(const YAML::Node& config, std::string ifce_name, std::string name,
                   std::string desc, Implementation* parent)
        : m_config(config), m_ifce_name(ifce_name), m_name(name), m_desc(desc),
          m_id(config["id"].as<std::string>("_default_id")), m_parent(parent), m_params(config){};

    Implementation(const YAML::Node& config, std::string ifce_name, std::string name,
                   std::string desc, std::string id, Implementation* parent)
        : m_config(config), m_ifce_name(ifce_name), m_name(name), m_desc(desc), m_id(id),
    ...
    /**
     * @brief     Performs initialization of the implementation object with the supplied
     * configuration.
     *
     */
    virtual void init() = 0;
    ...
    template <class Interface_t> Interface_t* cast_parent()
    {
    ...
    template <class Interface_t> Interface_t* create_child_ifce(const YAML::Node& config)
    {
        return create_child<Interface_t>(config, "");
    }

    template <class Interface_t> Interface_t* create_child_ifce()
    {
    ...
    template <class Interface_t, class Implementation_t> Implementation_t* create_child_impl()
    {
    ...
    template <typename T> StatWrapper<T>& register_stat(T& val)
    {
        StatWrapper<T>* s = new StatWrapper<T>(val, *this, m_stats);
        return *s;
    };
    template <typename T> StatWrapper<T>& register_stat(std::vector<T>& val)
    {
        StatWrapper<T>* s = new StatWrapper<T>(val, *this, m_stats);
        return *s;
    };
...
#define RAMULATOR_REGISTER_IMPLEMENTATION(_ifce_class, _impl_class, _name, _desc)                  \
  ...
  protected:                                                                                       \
    ...
    static Implementation* make_##_impl_class(const YAML::Node& config, Implementation* parent)    \
    {                                                                                              \
...
template <class T> class TopLevel
{
  ...
  public:
    ...
    template <class Ifce_t> Ifce_t* get_ifce(std::string desired_id = "")
    {
