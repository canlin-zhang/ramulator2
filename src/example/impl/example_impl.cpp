...
{
    RAMULATOR_REGISTER_IMPLEMENTATION(ExampleIfce, ExampleImpl, "ExampleImplementation",
  ...
  public:
    ...
    {
        return "I am an example implementation.";
    };
    void init() override
    {
        ...
        // int gi =
        // m_params.group("example_group").param<int>("in_group_param").default_val(3).required();
        ...
        return;
        // int ggi =
        // m_params.group("l1group").group("l2group").group("l3group").param<int>("nested_group_param").required();
...
class ExampleImplDerived : public ExampleImplBase, public Implementation
{
    RAMULATOR_REGISTER_IMPLEMENTATION(
        ExampleIfce, ExampleImplDerived, "ExampleImplDerived",
        "An example of an implementation class derived from a common implementation base class.")
...
class ExampleSerializableImpl : public ExampleIfce,
                                public Implementation,
                                public Serializable<ExampleSerializableImpl>
{
    RAMULATOR_REGISTER_IMPLEMENTATION(ExampleIfce, ExampleSerializableImpl,
                                      "ExampleSerializableImpl",
                                      "An example of an implementation class that is serializable.")
  public:
    int foo_int;
    std::string foo_str;
    ...
    struct foo_struct
    {
        ...
        long tag;
        bool lock;
        foo_struct(long addr, long tag, bool lock) : addr(addr), tag(tag), lock(lock)
        {
        }
    };
    ...
    void serialize() override
    {
        ...
        for (auto it = foo_lines.begin(); it != foo_lines.end(); ++it)
        {
            ...
            {
                out_file << it->first << "," << it2->addr << "," << it2->tag << "," << it2->lock
                         << "," << std::endl;
            }
        }
        out_file.close();
